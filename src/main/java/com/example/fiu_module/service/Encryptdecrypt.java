...
public class Encryptdecrypt {
    ...
    private String responseTopic = "aa_redirection_response";
     ...
     public String encryption(String strToEncrypt, String salt, String aggregator) {
        try {
            IvParameterSpec iv = new IvParameterSpec(new byte[16]); // Zero IV for simplicity
            SecretKeySpec secretKeySpec = generateSecretKey(salt, aggregator);
    ...
    public String decryption(String strToDecrypt, String salt, String aggregator) {
        try {
            IvParameterSpec iv = new IvParameterSpec(new byte[16]); // Zero IV for simplicity
            SecretKeySpec secretKeySpec = generateSecretKey(salt, aggregator);
    ...
    private SecretKeySpec generateSecretKey(String salt, String aggregator) throws Exception {
        String webSecretKey = getWebSecretKeyFromEnv(aggregator);

        if (webSecretKey == null) {
            throw new IllegalArgumentException("No webSecretKey found for aggregator: " + aggregator);
        }

        byte[] decodedBytes = Base64.getDecoder().decode(webSecretKey);
        KeySpec spec = new PBEKeySpec(new String(decodedBytes).toCharArray(), salt.getBytes(CHARSET), ITERATION_COUNT, KEY_SIZE);
        SecretKey tmp = SecretKeyFactory.getInstance(KEY_ALGO).generateSecret(spec);
        return new SecretKeySpec(tmp.getEncoded(), AES_ALGO);
    }

    private String getWebSecretKeyFromEnv(String aggregator) {
        String keyEnvVariable = aggregator.toUpperCase() + "_webSecretKey";
        return System.getenv(keyEnvVariable);
    }
